 ...
 (function( $ ) {
   "use strict";
    ...
    geckoShopify.swatchesOnBGGrid = function() {
       ...
       var swatches_js = function ( $this ) {
            ...
            // }
            image.attr('data-bgset', imagebg).attr('data-id', id);
            if ( nt_settings.pr_curent == '1') return;
   ...
   geckoShopify.cartPosDropdown = function() {
    ...
    geckoShopify.wishlistLocal = function () {
      ...
      // remove wishlist
      body.on('click', '.wis_remve', function (e) {
        e.preventDefault();
        var _this = $(this),
            id = 'id:'+_this.data('id'),
            ls = localStorage.getItem('nt_wis');
        ...
        $.ajax({
          ...
          success: function (data, status) {
             ...
             if (data.status == 'success' && status == 'success') {
                ...
                if (arr_wis_id.text() == '') {
                  arr_wis_id.text('id:'+id);
                } else {
                  arr_wis_id.append(',id:'+id);
                }
          ...
          success: function (data, status) {
             ...
             if (data.status == 'success' && status == 'success') {
                ...
                var txt_arr = arr_wis_id.text(),
                    last_txt = txt_arr.replace(",id:"+id,"").replace("id:"+id,"");
